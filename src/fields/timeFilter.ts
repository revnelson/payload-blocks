...
const timezone: (props?: {
  enumPrefix?: string
  enumName?: ({ tableName }: { tableName: string | undefined }) => string
}) => Field = (props) => ({
  name: 'timezone',
  type: 'select',
  enumName:
    props?.enumName || props?.enumPrefix
      ? ({ tableName }) => {
          if (props?.enumName) {
            return props.enumName({ tableName })
          }
          if (props?.enumPrefix) {
            return `${props.enumPrefix}_tz`
          }

          return `${tableName}_tz`
        }
      : undefined,
...
export const timeFilter: Field = {
  name: 'time',
  interfaceName: 'TimeFilter',
  type: 'group',
  label: false,
  fields: [
    {
